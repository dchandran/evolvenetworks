...
{	
	...
	void MainWindow::setupMassActionNetwork(QTreeWidget* treeWidget)
	{
		...
		child->setToolTip(0,"Proportion of the reactions that represent flux into the system (no reactant)");
		treeWidget->setItemWidget(child,1,doubleSpinBox = new QDoubleSpinBox);
		doubleSpinBox->setRange(0.0,1.0);
		doubleSpinBox->setDecimals(3);
		doubleSpinBox->setSingleStep(0.01);		
		...
		child->setToolTip(0,"Proportion of the reactions that represent flow out of the system (no product)");
		treeWidget->setItemWidget(child,1,doubleSpinBox = new QDoubleSpinBox);
		doubleSpinBox->setRange(0.0,1.0);
		doubleSpinBox->setDecimals(3);
		doubleSpinBox->setSingleStep(0.01);		
	...
	void MainWindow::setupEnzymeNetwork(QTreeWidget* treeWidget)
	{
		...
		enzyme->addChild(child = new QTreeWidgetItem);
		child->setText(0,"Alpha range");
		child->setToolTip(0,"Initial range for the Alpha parameter in log scale (base 2)");
		treeWidget->setItemWidget(child,1,doubleSpinBox = new QDoubleSpinBox);
		doubleSpinBox->setRange(0.0,1.0);
		doubleSpinBox->setDecimals(3);
		doubleSpinBox->setSingleStep(0.01);		
		connect(doubleSpinBox,SIGNAL(valueChanged(double)),this,SLOT(enzyme_init_max_alpha(double)));
	...
	QWidget * MainWindow::setupGAOptions()
	{
		...
		treeWidget->setItemWidget(code,1,new QLineEdit("run_net_ga.c"));
		
		QTreeWidgetItem * output = new QTreeWidgetItem;
		output->setText(0,"Output file");
		...
		treeWidget->setItemWidget(output,1,new QLineEdit("output.txt"));
		...
		seed->setText(0,"Seed");
		generation->setToolTip(0,"Set the random number generator seed. This is used for repeating the same experiment.");
		treeWidget->addTopLevelItem(seed);
		treeWidget->setItemWidget(seed,1,new QLineEdit("1.0"));
	...
	QWidget * MainWindow::setupEditor()
	{
		...
		Tinkercell::CodeEditor * codeEditor = new Tinkercell::CodeEditor;
