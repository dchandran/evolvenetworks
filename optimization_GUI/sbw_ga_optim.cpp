...
void rates_function(double t, double * y, double * rates, void * data)
{
	int i;
	sim_data * u = (sim_data*)data;
	for (i=0; i < u->numSpecies; ++i)
		u->varValues[i] = y[i];

	for (i=0; i < u->numReactions; ++i)
...
float Objective1(GAGenome & x)
{
	...
	float score = 0.0;
	model_data * u = (model_data*)g.ga->userData();

	sim_data sim;
	sim.varValues = new double[u->numSpecies];
	sim.rateEqns = u->rateEqns;

	for (int i=0; i < u->rateEqns.size(); ++i)
	{
		for (int j=0; j < u->paramNames.size(); ++j)
			sim->rateEqns[i].DefineConst(u->paramNames[j],g.gene(j));

		for (int j=0; j < u->varNames.size(); ++j)
			sim->rateEqns[i].DefineVar(u->varNames[j],&sim.varValues[j]);
	}

	double * y = ODEsim2(u->numSpecies, u->numReactions, u->N , &rates_function, u->varValues, u->startTime, u->endTime, u->stepSize, &sim);
	free(sim.varValues);

	if (y)
	{
		float diff, sumOfSq = 0.0;
		int numCols = 0;
		int numPoints = (int)((u->endTime - u->startTime)/(u->stepSize));
		for (int i=0; i < u->numSpecies; ++i)
			if (u->targetData[i].size() > 0)
			{
				for (int j=0; j < numPoints; ++j)
					diff = u->targetData->at(i).at(j) - y [ j*(u->numSpecies+1) + i ];
				sumOfSq += diff*diff;
				++numCols;
			}
		if (numCols < 1)
			...
			score = sumOfSq/(numCols*numPoints);
		free(y);
	}
	else
	{
